...
def scrap(options_ticker):
    ...
    for n, k in enumerate(hashtags):
        for i, tweet in enumerate(sntwitter.TwitterSearchScraper(f'{k} lang:en near:maroc').get_items()):
...
def snscrap_function(options_ticker,Limit,Since,Until):
    ...
    if Since == None and Until == None:
        ...
        for n,k in enumerate(hashtags):
            for i, tweet in enumerate(sntwitter.TwitterSearchScraper(f'{k} since:{Since} until:{Until} lang:en near:maroc' ).get_items()):
                if int(Limit) != 0 :
                    if i > int(Limit):
                        break
                tweets.append([k,tweet.date,tweet.id,tweet.content])
...
def scrapData():
    ...
    ListSearch=['#Coronavirus','#Covid19']
    options_ticker=list()
    if not op:
        with st.form('Scraping section'):
            col1,col2,col3,col4=st.columns((2,0.5,2,2))
            with col1:
                options_ticker=st.multiselect(label="Select hashtags",options=ListSearch,default='#Coronavirus',help="By default,the multiselect takes #Coronavirus for search!")
        ...
        options_details=st.multiselect(label="Select hashtags",options=list_details,
                                        default='Limit',
                                        help="By default,the multiselect takes #Coronavirus for search!")
        with st.form('Scraping section'):
                col1,col2,col3,col4=st.columns((1.5,0.8,2,2))
                options_ticker=col1.multiselect(label="Select hashtags",options=ListSearch,default='#Coronavirus',help="By default,the multiselect takes #covid19 for search!")
...
if __name__ == '__main__':
    ...
    if menu_id =='Home':
        st.title("Sentiments Analysis of Moroccans during coronavirus (Twitter)")
        ...
        col1.write("") 
        col1.write("")
        col1.write("We are interested in this work of sentiment analysis based on Twitter tweets. This process begins with the collection of tweets using snscrap followed by a pre-processing phase of the text in order to extract frequent words. Then categorize them into three groups, namely positive sentiments, negative sentiments and neutral sentiments.")
        ...
        st.subheader('Sentiment analysis results in Morocco')
        ...
        c22.table(tweets[tweets['Hashtag']== '#Coronavirus'])
        c22.table(tweets[tweets['Hashtag']== '#Covid19'])
